import { store, subscribe, setDraftSlip, clearDraftSlip, bookTicket, submitCustomerTicket, scorersForFixture, cardsForFixture, computeStandings } from './store.js';
...
// 'agent' = shared cash-booking account (existing behaviour, unchanged).
// 'customer' = self-service: submits a ticket as pending_payment instead
// of booking+printing, and only ever sees their own ticket history.
let currentRole = 'agent';
let ui = { tab: 'fixtures', moreOpen: false, sheetOpen: false, historyQuery: '', detailFixtureId: null, submitted: null };
let unsubscribe = null;

export function mountAgentView(root, opts = {}) {
  currentRole = opts.role || 'agent';
  ui = { tab: 'fixtures', moreOpen: false, sheetOpen: false, historyQuery: '', detailFixtureId: null, submitted: null };
  render(root);
...
function isCustomer() { return currentRole === 'customer'; }

// Fixtures (browse + book) and Ticket History/My Tickets are what someone
// needs on every visit, so they always stay on the dashboard; Standings is
// checked far less often and collapses into the hamburger "More" menu on
// phone-width screens (see js/nav.js + .tabs-dashboard in css/styles.css).
function tabsList() {
  return [
    { key: 'fixtures', label: 'Fixtures', core: true },
    { key: 'history', label: isCustomer() ? 'My Tickets' : 'Ticket History', core: true },
...
function paint(root) {
  root.innerHTML = `
    <div>
      ...
      ${tabNavHtml(tabsList(), ui.tab, ui.moreOpen, 'tab-btn')}

      <div class="agent-layout">
        <div class="agent-main">
          ${ui.tab === 'fixtures'
            ? (ui.submitted ? submittedTicketHtml(ui.submitted)
        ...
        <div class="betslip-desktop">${ui.submitted ? '' : betslipHtml()}</div>
      </div>
...
// Shown right after a self-service customer submits a ticket — no print,
// just the ticket ID and where to send the money (already shown as a
// banner above too). Payment is confirmed manually by an admin afterward.
function submittedTicketHtml(ticket) {
  return `
    <div class="settings-card" style="text-align:center;padding:24px 16px;">
      <p style="font-size:15px;color:var(--pitch-700);font-weight:700;margin:0 0 4px;">Ticket Submitted</p>
      <p style="font-size:24px;font-family:monospace;font-weight:700;margin:0 0 12px;">${escapeHtml(ticket.id)}</p>
      <p style="font-size:13px;color:var(--gray-800);margin:0 0 4px;">
        Stake: <strong>${ticket.stake_amount.toFixed(2)}</strong> · Potential Return: <strong>${ticket.potential_return.toFixed(2)}</strong>
      </p>
      <p style="font-size:12px;color:var(--gray-500);margin:12px 0 0;">
        Transfer the stake to the account above, using <strong>${escapeHtml(ticket.id)}</strong> as the
        reference if you can. Your ticket shows as <strong>Pending Payment</strong> until an admin confirms
        it landed — check "My Tickets" for the status.
      </p>
      <button type="button" class="btn-secondary tap-target" id="submitted-ok-btn" style="margin-top:16px;">Okay</button>
    </div>
  `;
}
...
function fixtureDetailHtml(fx) {
  ...
  const cardOptions = cardsForFixture(fx);
  ...
  return `
    <div class="fixture-detail">
      ...
      ${cardOptions.length > 0 ? `
        <div class="market-group">
          <div class="market-group-title">Player to be Booked</div>
          <p class="market-group-hint">Tap as many players as you like — not limited to one.</p>
          <div class="custom-market-grid">
            ${cardOptions.map((cd) => {
              const selected = isSelected('CARD', 'card_id', cd.id);
              return `
                <button class="custom-market-btn ${selected ? 'selected' : ''} ${disabled ? 'disabled' : ''}"
                  ${disabled ? 'disabled' : ''}
                  data-action="toggle-card" data-fixture="${fx.id}" data-card="${cd.id}">
                  <span class="cm-label">${escapeHtml(cd.name)} (${cd.card_type === 'red' ? 'Red' : 'Yellow'})</span>
                  <span class="odds-val">${cd.odds.toFixed(2)}</span>
                </button>
              `;
            }).join('')}
          </div>
...
function betslipHtml() {
  ...
  const customer = isCustomer() ? currentUser() : null;
    ...
    <div class="betslip-panel">
      ${isCustomer()
        ? `<div>
            ...
            <p style="margin:2px 0 0;font-weight:600;">${escapeHtml(customer?.name || 'You')}</p>
          </div>`
        : `<div>
          ...
          : `<ul style="list-style:none;padding:0;margin:4px 0 0;">
              ${selections.map((s) => `
                <li class="selection-item">
                  ...
                  <button class="remove-btn" data-action="remove-selection" data-fixture="${s.fixture_id}" data-market="${s.market}" data-custom-market="${s.custom_market_id || ''}" data-player="${s.player_id || ''}" data-card="${s.card_id || ''}" aria-label="Remove selection">&times;</button>
                </li>
              `).join('')}
            </ul>`
        }
      ...
      <button class="btn-primary tap-target book-print-btn">${isCustomer() ? 'Submit Ticket' : 'Book & Print'}</button>
    </div>
  `;
}

function paymentStatusBadgeHtml(t) {
  if (!t.payment_status) return ''; // agent-booked ticket — no payment claim to show
  const label = { pending_payment: 'Pending Payment', confirmed: 'Payment Confirmed', rejected: 'Payment Rejected' }[t.payment_status] || t.payment_status;
  return `<span class="payment-status-badge payment-status-${t.payment_status}">${escapeHtml(label)}</span>`;
}
...
function ticketHistoryHtml() {
  ...
  const base = isCustomer()
    ? store.state.tickets.filter((t) => t.user_id === customer?.id)
    : store.state.tickets;
  ...
  return `
    ...
    ${filtered.length === 0
      ? `<p style="text-align:center;color:var(--gray-400);padding:40px 0;">${isCustomer() ? "You haven't submitted any tickets yet." : 'No tickets found.'}</p>`
      : filtered.map((t) => `
        <div class="ticket-item ticket-${t.status}">
          ...
          ${paymentStatusBadgeHtml(t)}
          ${t.payment_status === 'rejected' && t.rejection_reason ? `<p style="font-size:12px;color:var(--gray-500);margin:4px 0;">Reason: ${escapeHtml(t.rejection_reason)}</p>` : ''}
          ...
          ${!isCustomer() ? `<button class="btn-secondary tap-target reprint-btn" data-action="reprint-ticket" data-ticket="${t.id}">🖨️ Reprint Slip</button>` : ''}
        </div>
      `).join('')
    }
  `;
...
function bindEvents(root) {
  ...
  root.querySelectorAll('[data-action="toggle-card"]').forEach((btn) => {
    btn.addEventListener('click', () => {
      const fixtureId = btn.dataset.fixture;
      const cardId = btn.dataset.card;
      const fixture = store.state.fixtures.find((f) => f.id === fixtureId);
      if (!fixture || fixture.status !== 'open') return;
      const cardOption = (store.state.cards || []).find((cd) => cd.id === cardId);
      if (!cardOption) return;

      const key = (s) => s.fixture_id === fixtureId && s.market === 'CARD' && s.card_id === cardId;
      const already = store.draftSlip.selections.some(key);
      ...
      setDraftSlip((slip) => {
        ...
        if (already) {
          ...
          selections = [...slip.selections, {
            fixture_id: fixtureId,
            market: 'CARD',
            card_id: cardOption.id,
            odds: cardOption.odds,
            label: `${fixture.home_team} vs ${fixture.away_team}`,
            marketLabel: `${cardOption.card_type === 'red' ? 'Red' : 'Yellow'} Card — ${cardOption.name}`,
          }];
        }
        return { ...slip, selections };
      });
  ...
  const submittedOk = root.querySelector('#submitted-ok-btn');
  if (submittedOk) submittedOk.addEventListener('click', () => { ui.submitted = null; ui.tab = 'history'; render(root); });
}
...
async function handleBook(root, btn) {
  const stakeNum = parseFloat(store.draftSlip.stake) || 0;
  ...
  if (isCustomer()) {
    ...
    if (!user) return showError('You need to be signed in to submit a ticket.');
    ...
    const ticket = await submitCustomerTicket(user, selections, stakeNum);
    btn.disabled = false;
    btn.textContent = originalLabel;

    if (!ticket) return; // the toast from submitCustomerTicket already explains what happened
  ...
  const ticket = await bookTicket(customerName, selections, stakeNum);
  btn.disabled = false;
  btn.textContent = originalLabel;

  if (!ticket) return; // the toast from bookTicket already explains what happened
