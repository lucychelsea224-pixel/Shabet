...
export function importTicket(backupTicket, idMap) {
  ...
  if (isSupabaseConfigured) {
    ...
    }).then(({ error }) => error && console.warn('Supabase: failed to import ticket', error));
  }
}
...
export function updateLeagueName(name) {
  ...
  if (isSupabaseConfigured && store.state.league.id) {
    supabase.from('leagues').update({ name }).eq('id', store.state.league.id)
      .then(({ error }) => error && console.warn('Supabase: failed to update league name', error));
  }
}
...
export function updateFixture(fixtureId, patch) {
  ...
  if (isSupabaseConfigured) {
    ...
    if (Object.keys(fixtureColumns).length > 0) {
      supabase.from('fixtures').update(fixtureColumns).eq('id', fixtureId)
        .then(({ error }) => error && console.warn('Supabase: failed to update fixture', error));
    }
  }
}
...
export function updatePlayer(playerId, patch) {
  ...
  if (isSupabaseConfigured) {
    supabase.from('players').update(patch).eq('id', playerId)
      .then(({ error }) => error && console.warn('Supabase: failed to update team', error));
  }
}
...
export function addScorer(team, name, odds) {
  if (isSupabaseConfigured) {
    ...
    supabase.from('scorers').insert({ league_id: store.state.league.id, team_id: teamId, name, odds }).select().single()
      .then(({ data, error }) => {
        if (error) { console.warn('Supabase: failed to add scorer', error); return; }
        setState((s) => ({
          ...s,
...
export function updateScorer(scorerId, patch) {
  ...
  if (isSupabaseConfigured) {
    ...
    supabase.from('scorers').update(dbPatch).eq('id', scorerId)
      .then(({ error }) => error && console.warn('Supabase: failed to update scorer', error));
  }
}
...
export function removeScorer(scorerId) {
  ...
  if (isSupabaseConfigured) {
    supabase.from('scorers').delete().eq('id', scorerId)
      .then(({ error }) => error && console.warn('Supabase: failed to remove scorer', error));
  }
}
...
export function addCustomMarket(fixtureId, label, odds) {
  if (isSupabaseConfigured) {
    ...
    supabase.from('custom_markets').insert({ fixture_id: fixtureId, label, odds }).select().single()
      .then(({ data, error }) => {
        if (error) { console.warn('Supabase: failed to add custom market', error); return; }
        setState((s) => ({
          ...s,
...
export function updateCustomMarket(fixtureId, marketId, patch) {
  ...
  if (isSupabaseConfigured) {
    supabase.from('custom_markets').update(patch).eq('id', marketId)
      .then(({ error }) => error && console.warn('Supabase: failed to update custom market', error));
  }
}
...
export function removeCustomMarket(fixtureId, marketId) {
  ...
  if (isSupabaseConfigured) {
    supabase.from('custom_markets').delete().eq('id', marketId)
      .then(({ error }) => error && console.warn('Supabase: failed to remove custom market', error));
  }
}
...
export function bookTicket(customerName, selections, stakeAmount) {
  ...
  if (isSupabaseConfigured) {
    ...
    }).then(({ error }) => error && console.warn('Supabase: failed to save ticket', error));
  }
...
export function settleFixture(fixtureId, homeScore, awayScore, scorerIds = [], customMarketWinIds = []) {
  ...
  if (isSupabaseConfigured) {
    ...
    }).then(({ error }) => error && console.warn('Supabase: failed to settle fixture', error));
  }
}
