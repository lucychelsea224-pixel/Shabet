...
// League table: Played / Won / Drawn / Lost / Points / Goals For:Against,
// built from every completed fixture's final score. 3 points for a win,
// 1 for a draw, standard tiebreak order (points, then goal difference,
// then goals scored).
export function computeStandings() {
  const table = new Map();
  for (const p of store.state.players) {
    table.set(p.player_name, {
      team: p.player_name, played: 0, won: 0, drawn: 0, lost: 0, goalsFor: 0, goalsAgainst: 0, points: 0,
    });
  }

  function ensureRow(name) {
    if (!table.has(name)) {
      table.set(name, { team: name, played: 0, won: 0, drawn: 0, lost: 0, goalsFor: 0, goalsAgainst: 0, points: 0 });
    }
  ...
  for (const fx of store.state.fixtures) {
    ...
    if (fx.home_score > fx.away_score) {
      ...
      away.lost += 1;
    } else if (fx.away_score > fx.home_score) {
