...
export function updatePaymentAccount(patch) {
  ...
  if (isSupabaseConfigured && store.state.league.id) {
    supabase.from('leagues').update(patch).eq('id', store.state.league.id).select()
      .then((res) => reportIfWriteFailed('update payment account', res));
  }
}
...
// README "How money works".
export function submitCustomerTicket(user, selections, stakeAmount) {
  const totalOdds = selections.reduce((acc, sel) => acc * sel.odds, 1);
  ...
  if (isSupabaseConfigured) {
    ...
    }).then(({ error }) => error && (console.warn('Supabase: failed to submit ticket', error), showToast(`Couldn't submit ticket ${ticket.id}: ${error.message}`)));
  }
  return ticket;
}
...
export function confirmTicketPayment(ticketDisplayId) {
  ...
  if (isSupabaseConfigured) {
    supabase.from('tickets').update({ payment_status: 'confirmed' }).eq('display_id', ticketDisplayId).select()
      .then((res) => reportIfWriteFailed('confirm payment', res));
  }
}
...
export function rejectTicketPayment(ticketDisplayId, reason) {
  ...
  if (isSupabaseConfigured) {
    supabase.from('tickets').update({ payment_status: 'rejected', rejection_reason: reason || '' }).eq('display_id', ticketDisplayId).select()
      .then((res) => reportIfWriteFailed('reject payment', res));
  }
}

const PENDING_SETTLEMENTS_KEY = 'shabet_pending_settlements_v1';
...
function savePendingSettlements(list) {
  try {
    localStorage.setItem(PENDING_SETTLEMENTS_KEY, JSON.stringify(list));
  } catch (e) {
    console.warn('Could not persist pending settlements queue.', e);
  }
  notify(); // let the Settlement tab's "pending sync" badge re-render
}

export function getPendingSettlementsCount() {
  return getPendingSettlements().length;
}

function queueSettlement(entry) {
  const list = getPendingSettlements().filter((e) => e.fixtureId !== entry.fixtureId);
  list.push({ ...entry, queuedAt: new Date().toISOString() });
  savePendingSettlements(list);
}

// Retries every settlement that previously failed to reach Supabase (e.g.
// "TypeError: Failed to fetch" from a dropped connection) — called on
// startup, whenever the browser comes back online, and from the manual
// "Retry Now" button in Admin > Settlement. Each one that succeeds is
// removed from the queue; failures stay queued for the next attempt.
export async function retryPendingSettlements() {
  const pending = getPendingSettlements();
  if (pending.length === 0) return;

  // Re-apply every queued result to the local view immediately — a page
  // reload re-fetches the (still-unsettled) server state and would
  // otherwise make these look "open" again while the network retry below
  // is in flight or still failing.
  setState((s) => pending.reduce(
    (acc, e) => settleFixtureLocally(acc, e.fixtureId, e.homeScore, e.awayScore, e.scorerIds || [], e.customMarketWinIds || []),
    s
  ));
  ...
  for (const entry of pending) {
    const { error } = await supabase.rpc('settle_fixture', {
      p_fixture_id: entry.fixtureId,
      p_home_score: entry.homeScore,
      p_away_score: entry.awayScore,
      p_scorer_ids: entry.scorerIds || [],
      p_custom_market_win_ids: entry.customMarketWinIds || [],
    });
    if (error) {
      console.warn('Supabase: retry of queued settlement failed', entry.fixtureId, error);
      stillPending.push(entry);
    } else {
      showToast(`Synced a previously-failed settlement (fixture settled ${entry.homeScore}-${entry.awayScore}).`);
    }
  }
  savePendingSettlements(stillPending);
}
  ...
  if (isSupabaseConfigured) {
    supabase.rpc('settle_fixture', {
      ...
      if (error) {
        ...
        // (see the retry block near fetchAllFromSupabase below).
        queueSettlement({ fixtureId, homeScore, awayScore, scorerIds, customMarketWinIds });
        showToast(
...
async function fetchAllFromSupabase() {
  ...
  const [{ data: players }, { data: fixtures }, { data: scorers }, { data: tickets }, { data: profiles }] = await Promise.all([
