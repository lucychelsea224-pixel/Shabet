...
## Self-service upgrade — design notes

The original build required a human agent to physically book every ticket. The
self-service upgrade (in progress / see `shabet-self-service-spec.md` for the full
technical spec) lets customers who aren't physically near an agent register their own
account and submit their own ticket — but keeps the "money never touches the app"
principle intact:

1. A customer builds a ticket in the app and submits it. The ticket is created with
   `payment_status = 'pending_payment'` and a ticket ID — nothing has been paid yet.
2. The app shows the operator's bank account details. The customer transfers the
   money **directly to that bank account**, the same way they'd hand cash to an agent
   — the app has no part in moving that money.
3. The operator checks their own bank account/app (outside Shabet) to see the
   transfer actually landed, then goes to Admin → Payment Approval Queue inside
   Shabet and clicks **Confirm Payment** (or **Reject**, if it never came through).
4. Only after that manual confirmation does the ticket become active and eligible for
   settlement — exactly like an agent-booked ticket from that point on.
...
confirm none of that machinery exists — the only money-adjacent tables are `tickets`
(records a stake amount, doesn't hold it) and, once built, `payment_accounts` (just
displays the operator's own bank details as text) and `admin_logs` (an audit trail of
who confirmed/rejected what, for accountability).
