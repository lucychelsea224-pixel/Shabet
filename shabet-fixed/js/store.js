...
// Human-readable one-liner for a queued write, used by the "not yet
// synced" list in Admin > Settlement so a stuck item can actually be
// identified instead of just counted.
function describePendingWrite(w) {
  switch (w.kind) {
    case 'settle_fixture': return `Settle fixture (score ${w.payload.homeScore}-${w.payload.awayScore})`;
    case 'update_league': return 'League name / payment account update';
    case 'update_fixture': return 'Fixture edit';
    case 'update_player': return 'Team edit';
    case 'add_scorer': return `Add scorer "${w.payload.name}"`;
    case 'update_scorer': return 'Scorer odds edit';
    case 'remove_scorer': return 'Remove scorer';
    case 'add_card': return `Add card "${w.payload.name}"`;
    case 'update_card': return 'Card odds edit';
    case 'remove_card': return 'Remove card';
    case 'add_custom_market': return `Add market "${w.payload.label}"`;
    case 'update_custom_market': return 'Market edit';
    case 'remove_custom_market': return 'Remove market';
    case 'confirm_payment': return `Confirm payment ${w.payload.ticketDisplayId}`;
    case 'reject_payment': return `Reject payment ${w.payload.ticketDisplayId}`;
    case 'delete_user': return 'Delete user';
    default: return w.kind;
  }
}

// Detail view backing the "not yet synced" list — never exposes raw
// payloads (e.g. full patches), just enough to identify and debug each
// stuck item.
export function getPendingWritesDetail() {
  return getPendingWrites().map((w, index) => ({
    index,
    kind: w.kind,
    label: describePendingWrite(w),
    queuedAt: w.queuedAt,
...
// Drops one stuck item from the queue without syncing it — for a write
// that will never succeed as-is (e.g. it referenced something that no
// longer exists), so it stops blocking/cluttering the retry list.
export function discardPendingWrite(index) {
  const list = getPendingWrites();
