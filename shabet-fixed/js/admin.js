import {
  ...
  addCard, updateCard, removeCard, cardsForFixture,
  ...
  getPendingSettlementsCount, retryPendingSettlements, getPendingWritesDetail, discardPendingWrite,
  applyDefaultCustomMarkets, applyDefaultCustomMarketsToAllFixtures,
} from './store.js';
import { tabNavHtml } from './nav.js';
import { showToast } from './toast.js';
import { withFocusPreserved } from './domFocus.js';
...
function render(root) {
  withFocusPreserved(root, () => paint(root));
}

function paint(root) {
  const pendingCount = getPendingSettlementsCount();
  root.innerHTML = `
    <div class="admin-wrap">
      ${tabNavHtml(TABS, ui.tab, ui.moreOpen, 'pill-tab')}
      ${pendingCount > 0 && ui.tab !== 'settlement' ? `
        <div class="pending-sync-bar" data-action="goto-settlement-for-sync">
          ⏳ ${pendingCount} change${pendingCount === 1 ? '' : 's'} not yet synced to the database — tap to view
        </div>
      ` : ''}
...
function fixtureConfiguratorHtml() {
  ...
  return `
    ...
    <div class="settings-card">
      <h3 style="margin:0 0 6px;font-size:15px;color:var(--pitch-800);">New default markets added?</h3>
      <p class="admin-subtext" style="margin:0 0 10px;">
        Fixtures created before a market was added to the default list won't have it automatically —
        this adds any missing default markets to every fixture below in one go, without touching
        ones you've already customized.
      </p>
      <button type="button" class="btn-secondary tap-target" data-action="apply-default-markets-all">
        Add Missing Default Markets to All Fixtures
      </button>
    </div>
    ...
    <div class="admin-card" data-fixture-card="${fx.id}">
      ...
      ${expanded ? `
        <div class="custom-market-admin-list">
          ...
          <button type="button" class="link-btn-inline tap-target" data-action="apply-default-markets" data-fixture="${fx.id}">
            + Add any missing default markets
          </button>
        </div>
      ` : ''}
    </div>
...
function settlementHtml() {
  const settleable = store.state.fixtures
    .filter((f) => f.status === 'live' || f.status === 'completed')
    .sort((a, b) => a.matchday - b.matchday);
  const matchdays = [...new Set(settleable.map((f) => f.matchday))];
  const pendingCount = getPendingSettlementsCount();
  ...
  return `
    ...
    ${pendingCount > 0 ? `
      <div class="settings-card" style="border:1px solid #f59e0b;background:#fffbeb;">
        <h3 style="margin:0 0 6px;font-size:15px;color:#92400e;">
          ${pendingCount} change${pendingCount === 1 ? '' : 's'} saved on this device, not yet synced
        </h3>
        <p class="admin-subtext" style="margin:0 0 10px;">
          The connection dropped when one or more actions were submitted — a settlement, an odds
          edit, a market, a payment confirmation, anything. They're already showing correctly here
          and will keep retrying automatically in the background (and whenever this device comes
          back online) — but you can force it now instead of waiting.
        </p>
        <button type="button" class="btn-secondary tap-target" data-action="retry-pending-settlements">Retry Now</button>
        <div style="margin-top:12px;display:flex;flex-direction:column;gap:8px;">
          ${getPendingWritesDetail().map((w) => `
            <div style="background:#fff;border:1px solid #fde68a;border-radius:8px;padding:8px 10px;font-size:13px;">
              <div style="display:flex;justify-content:space-between;gap:8px;align-items:baseline;">
                <strong>${escapeHtml(w.label)}</strong>
                <button type="button" class="link-btn" data-action="discard-pending-write" data-index="${w.index}"
                  style="font-size:12px;color:#92400e;">Discard</button>
              </div>
              ${w.lastError ? `<div style="color:#92400e;margin-top:4px;">Last error: ${escapeHtml(w.lastError)}</div>` : ''}
            </div>
          `).join('')}
        </div>
      </div>
    ...
    ${settleable.length === 0
      ? `<p style="text-align:center;color:var(--gray-400);padding:24px 0;">
          No matches are live or completed yet. Set a fixture to "live" or "completed" in the
          Fixtures tab first — a match has to be Live or Completed before it shows up here to settle.
        </p>`
      : matchdays.map((md) => `
          <div class="matchday-heading">Matchday ${md}</div>
          ${settleable.filter((fx) => fx.matchday === md).map((fx) => settleItemHtml(fx)).join('')}
        `).join('')
    }
  `;
}

function settleItemHtml(fx) {
  const eligibleScorers = scorersForFixture(fx);
  const confirmed = fx.scorers_confirmed || [];
  const eligibleCards = cardsForFixture(fx);
  const cardsConfirmed = fx.cards_confirmed || [];
  return `
            <div class="settle-item">
              ...
              ${eligibleCards.length > 0 ? `
                <div class="scorer-confirm-box">
                  <span class="field-label">Who got carded in this match?</span>
                  <div class="scorer-confirm-list">
                    ${eligibleCards.map((cd) => `
                      <label class="scorer-checkbox">
                        <input type="checkbox" data-card-confirm="${fx.id}" value="${cd.id}" ${cardsConfirmed.includes(cd.id) ? 'checked' : ''} />
                        ${escapeHtml(cd.name)} (${cd.card_type === 'red' ? 'Red' : 'Yellow'})
                      </label>
                    `).join('')}
                  </div>
                </div>
...
function cardsHtml() {
  ...
  return `
    <h2 class="admin-heading font-display">Cards Roster (Player to be Booked)</h2>
    <p class="admin-subtext">
      Same idea as Anytime Scorer, but for yellow/red cards — these are the exact names agents see
      in the Cards dropdown when booking a ticket. Add a player once per card type you want to
      offer (e.g. both "Yellow Card" and "Red Card" as separate bettable outcomes for the same
      player). Team is optional — leave it blank to offer a player on every fixture.
    </p>

    <div class="settings-card">
      <div class="scorer-add-grid">
        <input type="text" id="new-card-name" class="text-input tap-target" placeholder="Player name" />
        <select id="new-card-team" class="select-input tap-target">
          <option value="">No team (show on all fixtures)</option>
          ${teams.map((t) => `<option value="${escapeHtml(t)}">${escapeHtml(t)}</option>`).join('')}
        </select>
        <select id="new-card-type" class="select-input tap-target" style="max-width:110px;">
          <option value="yellow">Yellow</option>
          <option value="red">Red</option>
        </select>
        <input type="number" step="0.01" min="1" id="new-card-odds" class="number-input tap-target" placeholder="Odds" value="3.25" />
        <button class="btn-secondary tap-target" id="add-card-btn">Add Card</button>
      </div>
    </div>

    ${cards.length === 0
      ? '<p style="text-align:center;color:var(--gray-400);padding:24px 0;">No cards yet — add one above.</p>'
      : cards.map((cd) => `
        <div class="registry-item scorer-item">
          <input class="registry-name-input text-input tap-target" data-card-field="name" data-card="${cd.id}" value="${escapeHtml(cd.name)}" />
          <select class="select-input tap-target" data-card-field="team" data-card="${cd.id}" style="max-width:120px;">
            <option value="" ${!cd.team ? 'selected' : ''}>No team</option>
            ${teams.map((t) => `<option value="${escapeHtml(t)}" ${cd.team === t ? 'selected' : ''}>${escapeHtml(t)}</option>`).join('')}
          </select>
          <select class="select-input tap-target" data-card-field="card_type" data-card="${cd.id}" style="max-width:90px;">
            <option value="yellow" ${cd.card_type === 'yellow' ? 'selected' : ''}>Yellow</option>
            <option value="red" ${cd.card_type === 'red' ? 'selected' : ''}>Red</option>
          </select>
          <input type="number" step="0.01" min="1" class="number-input tap-target" style="max-width:80px;" data-card-field="odds" data-card="${cd.id}" value="${cd.odds}" />
          <button class="remove-btn tap-target" data-action="remove-card" data-card="${cd.id}" aria-label="Remove card">&times;</button>
        </div>
      `).join('')
    }
  `;
...
function bindEvents(root) {
  ...
  const retryBtn = root.querySelector('[data-action="retry-pending-settlements"]');
  if (retryBtn) retryBtn.addEventListener('click', () => retryPendingSettlements());

  root.querySelectorAll('[data-action="discard-pending-write"]').forEach((btn) => {
    btn.addEventListener('click', () => {
      const ok = window.confirm('Discard this change without syncing it to the database? This can\'t be undone.');
      if (ok) { discardPendingWrite(Number(btn.dataset.index)); render(root); }
    });
  });

  const pendingBar = root.querySelector('[data-action="goto-settlement-for-sync"]');
  if (pendingBar) pendingBar.addEventListener('click', () => { ui.tab = 'settlement'; ui.moreOpen = false; render(root); });
  ...
  root.querySelectorAll('[data-action="settle"]').forEach((btn) => {
    btn.addEventListener('click', () => {
      ...
      const cardIds = Array.from(card.querySelectorAll(`[data-card-confirm="${fixtureId}"]:checked`)).map((cb) => cb.value);
      settleFixture(fixtureId, home, away, scorerIds, customMarketWinIds, cardIds);
    });
  });
  ...
  // --- Cards roster (Player to be Booked) ---
  const addCardBtn = root.querySelector('#add-card-btn');
  if (addCardBtn) {
    addCardBtn.addEventListener('click', () => {
      const nameEl = root.querySelector('#new-card-name');
      const teamEl = root.querySelector('#new-card-team');
      const typeEl = root.querySelector('#new-card-type');
      const oddsEl = root.querySelector('#new-card-odds');
      const name = nameEl.value.trim();
      if (!name) { nameEl.focus(); return; }
      const odds = parseFloat(oddsEl.value) || 3.25;
      addCard(teamEl.value, name, typeEl.value, odds);
    });
  }

  root.querySelectorAll('[data-card][data-card-field]').forEach((el) => {
    el.addEventListener('change', () => {
  ...
  root.querySelectorAll('[data-action="remove-card"]').forEach((btn) => {
    btn.addEventListener('click', () => removeCard(btn.dataset.card));
  });
  ...
  root.querySelectorAll('[data-action="apply-default-markets"]').forEach((btn) => {
    btn.addEventListener('click', () => {
      const added = applyDefaultCustomMarkets(btn.dataset.fixture);
      showToast(
        added > 0 ? `Added ${added} missing default market${added === 1 ? '' : 's'}.` : 'This fixture already has every default market.',
        'success'
      );
    });
  });

  const applyAllBtn = root.querySelector('[data-action="apply-default-markets-all"]');
  if (applyAllBtn) {
    applyAllBtn.addEventListener('click', () => {
      const added = applyDefaultCustomMarketsToAllFixtures();
      showToast(
        added > 0 ? `Added ${added} missing default market${added === 1 ? '' : 's'} across all fixtures.` : 'Every fixture already has every default market.',
        'success'
      );
    });
  }
