...
- **Anytime Scorer is a dropdown of admin-entered names, not free text.** Admin > Anytime Scorer
  ...
  against that list.
- **Cards (Player to be Booked) works exactly like Anytime Scorer.** Admin > Cards manages a
  roster the same way — name, optional team, yellow or red, odds. Add the same player twice (once
  per card type) if you want to offer "Yellow Card" and "Red Card" as separate bettable outcomes.
  The fixture Details page shows a "Player to be Booked" group under Anytime Scorer; Settlement
  gets a matching "Who got carded in this match?" checklist next to the scorer one, confirmed the
  same way via `fixtures.cards_confirmed`.
...
## If the connection drops mid-save (offline resilience)

Every admin write — settling a match, editing odds, adding a market, confirming a payment,
renaming the league, anything — updates the screen immediately and *then* tries to save it to
Supabase. If that save fails (almost always "Failed to fetch" from a dropped connection, not a
real rejection), the change is remembered in a queue on that device (`shabet_pending_writes_v1` in
localStorage) and retried automatically: on the next page load, the moment the device reconnects
to the internet, and from a manual "Retry Now" button. A small amber bar appears at the top of
every Admin tab whenever anything is still waiting to sync, and tapping it jumps straight to
Admin > Settlement, where the full list and the Retry button live. Nothing in this queue is ever
silently dropped — the only way an entry disappears is that it actually reached the database.

The one exception is booking a ticket (`bookTicket`/`submitCustomerTicket` in `js/store.js`): that
one now properly *waits* for the database to confirm the save before saying "Ticket Submitted."
If it fails, the customer/agent is told plainly that nothing was booked, instead of the ticket
number being silently lost the way it used to be — see `assign_ticket_display_id()` in
`schema.sql`, which is also what makes every ticket number globally unique now (assigned by an
atomic database sequence, not guessed by whichever device happens to book first).
